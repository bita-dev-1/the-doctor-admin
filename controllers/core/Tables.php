...
function data_table($DB)
{
    ...
    foreach ($str_arr as $col_str) {
        ...
        $clean_order_col = trim(str_replace(array("_photo", "__action", "_state", "_BadgeState", "_stateId", "__enableRdv"), "", $col_for_order));
        if (empty($clean_order_col)) {
            $order_cols[] = 'id';
        } else {
            $order_cols[] = $clean_order_col;
        }
    ...
    if (!empty($_REQUEST['search']['value'])) {
        ...
        if (!empty($search_cols)) {
            $base_query .= " AND CONVERT(CONCAT_WS(' ', " . implode(",", $search_cols) . ") USING utf8mb4) LIKE ? ";
            $params[] = "%" . $search_value . "%";
        }
    }
    ...
    try {
        // 7. Total Count
        $count_sql = "SELECT COUNT(*) FROM ($base_query) AS count_table";
        $stmtCount = $DB->pdo->prepare($count_sql);
        $stmtCount->execute($params);
        $totalData = $stmtCount->fetchColumn();

        // 8. Order & Limit
        $order_column_index = $_REQUEST['order'][0]['column'];
        $order_column = $order_cols[$order_column_index] ?? 'id';
