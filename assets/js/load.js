...
const SITE_URL = $('.SITE_URL').val(), API_URL = $('.API_URL').val();
...
$(document).ready(function(){
    $('.select2').each(function () {
        ...
        if(typeof self.attr("data-express") === 'undefined' || self.attr("data-express") === false){
            self.select2({
            ...
            if(typeof self.attr("his_parent") === 'undefined' || self.attr("his_parent") === false){
                self.select2({
                    dropdownParent: self.parent(),
                    placeholder: self.attr('placeholder'),
                    ajax: {
                        ...
                        data: function (params) {
                            var query = {"searchTerm": params.term ,method:"select2Data", token: self.data('express') }
                            return query;
                        },
                    ...
                    let child = $(document).find(`.select2[his_parent="#${self.attr('name')}"]`);
                    if(child.length){
                        child.empty();
                    }
                    self.valid();
                ...
                if($(self.attr("his_parent")).find('option').length){
                    self.select2({
                        ...
                        ajax: {
                            ...
                            delay: 250,
                            data: function (params) {
                                var query = {"searchTerm": params.term ,method:"select2Data", token: self.data('express'), parent: $(self.attr("his_parent")).val() }
                                return query;
                            },
                            processResults: function (data) {
                                return {
...
$('input.picker').on('change', function (e) {
    e.preventDefault();
    e.stopPropagation();
    $(this).parents('body').find('.table.dataTable#codexTable').DataTable().ajax.reload();
});

function call_data_table(data){
    var dataButton = "", buttons = [];
    if(data.hasOwnProperty("button")){
        ...
        $.each( dataButton, function( index, value ){
            ...
            if(!value.hasOwnProperty("collection")){
                ...
                else if(value.hasOwnProperty("url")) singleBtn.action   = function ( e, dt, button, config ) {window.location = value['url']};
                
            }else{
                singleBtn.extend = 'collection';
                if(value.hasOwnProperty("class")) singleBtn.className   = value['class'];
                if(value.hasOwnProperty("text")) singleBtn.text         = feather.icons['external-link'].toSvg({ class: 'font-small-4 me-50' }) + value['text'];
                singleBtn.buttons = [];
   ...
   var  select  = $('#codexTable').parents('#ajax-datatable').find('#selectstate');
    $(select).on('change',function () {
        $(this).parents('#ajax-datatable').find('.dataTable.table').DataTable().ajax.reload();

    })
    
    var  selecttype  = $('#codexTable').parents('#ajax-datatable').find('#selecType');
   ...
   $('#codexTable').DataTable({
        ...
        "order": [[0, 'desc']],
        "language": {
            "url": SITE_URL+"/assets/js/dataTableLangue/"+(langDataTable.length ? langDataTable : 'fr')+".json"
        },
        "ajax":{
            url: SITE_URL+"/data",
            type:"POST",
...
$(document).on('click', '.delete-record', function(e){
    ...
    var data = {method:"deleteItem_table", table: self.parents('.table.dataTable').data('express'), id:self.data('id')};
...
function dataTable_exportItem(data){
    ...
    switch(data['role']){
        case 'csv':
            ...
            item.text           = feather.icons['file-text'].toSvg({ class: 'font-small-4 me-50' }) + data['text'];
            item.exportOptions  = data['exportOptions'];
        break;
        case 'excel':
            item.extend         = data['role'];
            item.text           = feather.icons['file'].toSvg({ class: 'font-small-4 me-50' }) + data['text'];
            item.exportOptions  = data['exportOptions'];
        break;
        case 'pdf':
            item.extend         = data['role'];
            item.text           = feather.icons['clipboard'].toSvg({ class: 'font-small-4 me-50' }) + data['text'];
            item.exportOptions  = data['exportOptions'];
        break;
        case 'copy':
            item.extend         = data['role'];
            item.text           = feather.icons['copy'].toSvg({ class: 'font-small-4 me-50' }) + data['text'];
            item.exportOptions  = data['exportOptions'];
        break;
        case 'print':
        default:
            item.extend         = data['role'];
            item.text           = feather.icons['printer'].toSvg({ class: 'font-small-4 me-50' }) + data['text'];
            item.exportOptions  = data['exportOptions'];
        break;
    }
    return item;
...
$(document).on('submit', '#codexForm, .codexForm', function(e){
    ...
    var data = { class: self.data('express'), data: $(this).find(':not(.excluded)').serializeArray() };

    if(typeof self.attr("data-update") === 'undefined' || self.attr("data-update") === false || self.attr('data-update') == "" || self.attr('data-update') == "UW0=") data.method= "postForm";
    else{ data.method= "updatForm"; data.object= $(this).attr('data-update'); data.attachemment= $(this).attr('data-sub-express'); }

    if(typeof self.attr("data-codex-id") !== 'undefined' && self.attr("data-codex-id") !== false){ data.codex_id = self.attr("data-codex-id"); }

    self.find('.form-switch .form-check-input:not(:checked)').each(function() {
        if(!isKeyInArray(data.data, this.name)){ data.data.push({ name: this.name, value: 0 }); }
    });

    if(self.hasClass('codexGallery'))
        data.multi = [{"name" : "multi"}];
    ...
    self.find('.codexFileUp .codexFileData:not(.excluded)').each(function() {
        if(self.hasClass('codexGallery')){
            ...
            data.data.push({ name: $(this).attr("data-name"), value: $(this).val() });
                        ...
                        Swal.fire({
                            ...
                            if(self.parents('.modal').length){
                               ...
                               if(self.parents('body').find('.table.dataTable').length){
                                    self.parents('body').find('.table.dataTable').DataTable().ajax.reload();
                                }
                            }else{
                                history.back(-1);
                            }
...
function moveUploadedFile(self, formData){   
    ...
    $.ajax({
      ...
      success: function(data){
        ...
        if(data.state == "true"){
            if(typeof self.parents('.codexFileUp').find('.codexInputFile').attr('multiple') === 'undefined' || self.parents('.codexFileUp').find('.codexInputFile').attr('multiple') === false ){
                
                if($('.codexAttachementMessage').length){
                    
                    var file = `<div class="attachement_item d-flex">
                        <span class="attachement_type">`+data.path[0].new.split('.').pop()+`</span>
                        ...
                        <p class="m-0" src="`+data.path[0].new+`">`+data.path[0].old+`</p></div>`;
                        $('.codexAttachementMessage').html(file);

                }else if(self.parents('.codexFiles').find('#codexPreviewImage').length){
                    var dataName = self.parents('.codexFileUp').find('.codexInputFile ').attr('name');

                    var img ='<div class="col-lg-3 col-md-4 col-sm-6 col-12"><span class="removePic">X</span><img src="'+data.path[0].new+'" alt="Preview Image">';
                    img +='<p>'+data.path[0].old+'</p><input type="hidden" class="codexFileData" data-name="'+dataName+'" value="'+data.path[0].new+'" /></div>  ';
                    self.parents('.codexFileUp').find('.codexMultiPreviewImage').html(img);

                } else if(self.parents('.codexFileUp').find('#codexPreviewImage').length){
                    self.parents('.codexFileUp').find('#codexPreviewImage').attr('src',  data.path[0].new);
                    self.parents('.codexFileUp').find('#codexPreviewImage').show();
                }
                self.parents('.codexFileUp').find('.codexFileData').val(data.path[0].new);
            }else{
                var dataName = self.parents('.codexFileUp').find('.codexInputFile ').attr('name'),
                img= "";
                $.each(data.path, function(index, item){
                    img ='<div class="col-lg-3 col-md-4 col-sm-6 col-12"><span class="removePic">X</span><img src="'+SITE_URL+'/'+item.new+'" alt="Preview Image">';
                    img +='<p>'+item.old+'</p><input type="hidden" class="codexFileData" data-name="'+dataName+'" value="'+item.new+'" /></div>  ';
                    self.parents('.codexFileUp').find('.codexMultiPreviewImage').append(img);
                });
            }
        }else{
            Swal.fire({
...
$(document).ready(function(){
    ...
    /********************[Begin]: signUp ********************/
        ...
        $(document).on('submit', '#codexSignUp', function(e){
            ...
            var data = { method: 'signUp', data: $(this).find(':not(.excluded)').serializeArray() };
            ...
            $('.codexFileUp .codexFileData:not(.excluded)').each(function() {
                if($(this).val() != ""){ data.data.push({ name: $(this).attr("data-name"), value: $(this).val() }); }
            });
...
function getSelectedSelect2(dataExpress, parent){
    if(parent == ""){
        ...
        return $.ajax({
            type: "POST",
            url:  SITE_URL+"/data",
            data: {method:"select2Data", token: dataExpress, parent: $(parent).val() },
            dataType: "json"
        });
    }
}
...
$(document).on('click', '.codexFire_modal', function(e){
    ...
    var self            =   $(this),
        ...
        $.ajax({
            type: "POST",
            url:  SITE_URL+"/data",
            data: {method: "dataById", class: selfClass,express: selfUpdate, id: selfId},
            dataType: "json",
            success: function(res) {  
              //  console.log(res);              
                if (res.state != "false") {
                    var $inputs = $(targetModal).find('#codexForm').find('input ,select ,textarea'), col = '';
                    ...
                    $inputs.each(function(){
                        ...
                        if($this.parents('.form-switch').length){
                            ...
                            if ($this.find("option[value='" + res[col] + "']").length) {
                                ...
                                var colItem = col, parentSelect = "";
                                //if(typeof $this.attr("his_parent") !== 'undefined' && $this.attr("his_parent") !== false){parentSelect = $this.attr("his_parent");}
                                getSelectedSelect2($this.attr('data-express'), parentSelect).done(function(data){
                                    var filtered = data.filter(function (el) {
                                        return el.id == res[colItem];
                                    });
                                    if(filtered.length != 0){
                                        var newOption = new Option(filtered[0].text, filtered[0].id, true, true);
                            ...
                            if($this.parents('.avatar-upload').length){
                                if(res[col] != null){
                                    $this.parents('.avatar-upload').find('#codexPreviewImage').attr('src', res[col]);
                                    $this.parents('.avatar-upload').find('#codexPreviewImage').addClass('d-block');
                                }
...
$(document).on('change', '.switch-table-record', function(e){
    ...
    var data = {method:"changeState", table: self.parents('.table.dataTable').data('express'), id:self.data('id')};
...
$(document).on('input', 'input[type=number]', function(e){
    ...
    $(this).val().replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$0');
