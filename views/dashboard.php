<?php 
    ...
    $is_super_admin = ($user_role === 'admin' && empty($user_cabinet_id)); // Variable to check for Super Admin

    // --- START: Dynamic WHERE clauses for multi-tenancy ---
    $users_where_clause = " WHERE deleted = 0 ";
    $patients_where_clause = " WHERE deleted = 0 ";
    $rdv_where_clause = " WHERE deleted = 0 ";

    // Cabinet Admin (admin with a cabinet_id) is filtered.
    ...
    // Doctors/Nurses only see their own rdv data, but patients from the same cabinet
    ...
    // For Super Admin, the conditions remain empty, showing all data.
    // --- END: Dynamic WHERE clauses ---


    $users = ['admins' => 0, 'doctors' => 0];
    // Only admins see user counts
    if ($user_role === 'admin') {
        $users_count_sql = "SELECT 
            ...
            FROM users " . str_replace('deleted = 0', 'deleted = 0 AND role != \'admin\'', $users_where_clause);
        
        $admins_count_sql = "SELECT COUNT(id) as total FROM users " . str_replace('deleted = 0', 'deleted = 0 AND role = \'admin\'', $users_where_clause);

        $users_count = $GLOBALS['db']->select($users_count_sql)[0] ?? ['doctors' => 0, 'nurses' => 0];
        $admins_count = $GLOBALS['db']->select($admins_count_sql)[0]['total'] ?? 0;
        
        $users['admins'] = $admins_count;
        $users['doctors'] = $users_count['doctors'];
    }
    ...
    // Specialities are global, so no filter needed
    $specialities = $GLOBALS['db']->select("SELECT COUNT(id) AS total FROM specialty WHERE deleted = 0")[0]['total'] ?? 0;
    
    $patients = $GLOBALS['db']->select("SELECT COUNT(id) AS total FROM patient " . $patients_where_clause)[0]['total'] ?? 0;
                                        ...
                                        <div class="row">
                                            <div class="col-xl-3 col-sm-6 col-12 mb-2 mb-xl-0">
                                                <div class="d-flex flex-row">
                                                    ...
                                                    <div class="my-auto">
                                                        <h4 class="fw-bolder mb-0"><?= $users['admins'] ?? 0; ?></h4>
                                            ...
                                            <div class="col-xl-3 col-sm-6 col-12 mb-2 mb-xl-0">
                                                <div class="d-flex flex-row">
                                                    ...
                                                    <div class="my-auto">
                                                        <h4 class="fw-bolder mb-0"><?= $users['doctors'] ?? 0; ?></h4>
                                                        <p class="card-text font-small-3 mb-0"><?= $GLOBALS['language']['doctors']; ?></p>
                                                    </div>
                                                </div>
                                            </div>
                                            
                                            <!-- START: Conditional display for Specialities -->
                                            ...
                                            <?php endif; ?>
                                            <!-- END: Conditional display for Specialities -->
